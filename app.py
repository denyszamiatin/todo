...
app = Flask(__name__)
app.secret_key = '@#$675@#$%^&tgrfedQDWAFS}{P[;LDC'
...
def index():
    if request.method == 'POST':
        ...
        m = pymongo.MongoClient()
        user = m.test.user.find_one({'login':login, 'passwd': passwd})
        if user:
            session_id = create_session_id()
            m.test.session.insert({
                'login': login,
...
def list_():
    ...
    m = pymongo.MongoClient()
    tasks = m.test.todo.find({'login': g.login})
...
def add():
    ...
    if request.method == 'POST':
        ...
        if desc and date:
            ...
            session['message'] = 'Task has been added successfully'
            return redirect('/list')
        else:
            message = 'Incorrect deadline or desription'
